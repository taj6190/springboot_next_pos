...
import { HiOutlinePlus, HiOutlinePencil, HiOutlineTrash, HiOutlineSearch, HiOutlinePhotograph, HiOutlineUpload } from "react-icons/hi";
...
export default function ProductsPage() {
  ...
  const emptyForm = { name: "", barcode: "", description: "", costPrice: "", sellingPrice: "", stock: "", minStock: "5", categoryId: "", brandId: "", unit: "pcs", weight: "", weightUnit: "g", soldByWeight: false, taxRate: "0" };
  const [form, setForm] = useState(emptyForm);
  const [brands, setBrands] = useState([]);

  useEffect(() => { fetchProducts(); fetchCategories(); fetchBrands(); }, [page]);
  ...
  const openEdit = (p) => {
    setEditing(p);
    setForm({ name: p.name, barcode: p.barcode || "", description: p.description || "", costPrice: p.costPrice, sellingPrice: p.sellingPrice, stock: p.stock, minStock: p.minStock, categoryId: p.categoryId || "", brandId: p.brandId || "", unit: p.unit || "pcs", weight: p.weight || "", weightUnit: p.weightUnit || "g", soldByWeight: p.soldByWeight || false, taxRate: p.taxRate || "0" });
    setModalOpen(true);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const body = { ...form, costPrice: parseFloat(form.costPrice), sellingPrice: parseFloat(form.sellingPrice), stock: parseInt(form.stock || "0"), minStock: parseInt(form.minStock || "5"), categoryId: form.categoryId ? Number(form.categoryId) : null, brandId: form.brandId ? Number(form.brandId) : null, taxRate: parseFloat(form.taxRate || "0"), weight: form.weight ? parseFloat(form.weight) : null };
    try {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ marginBottom: "1rem", position: "relative", maxWidth: 400 }}>
        ...
        <input className="input" placeholder="Search products..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ paddingLeft: "2.5rem" }} />
      </div>

      <div className="table-container">
        <table>
          <thead><tr><th style={{width:60}}>Image</th><th>Product</th><th>Brand</th><th>Category</th><th>Cost</th><th>Price</th><th>Stock</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            {products.map((p) => (
              <tr key={p.id}>
                <td>
                  <div style={{ width: 44, height: 44, borderRadius: 8, overflow: "hidden", background: "var(--bg-hover)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}
                ...
                <td><div><p style={{ fontWeight: 600, fontSize: "0.85rem" }}>{p.name}</p><p style={{ fontSize: "0.7rem", color: "var(--text-secondary)" }}>{p.sku}</p></div></td>
                ...
                <td><span className={`badge ৳{p.lowStock ? "badge-danger" : "badge-success"}`}>{p.stock}</span></td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit Product" : "Add Product"} width="580px">
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.75rem" }}>
            ...
            <div><label className="form-label">Tax %</label><input className="input" type="number" step="0.1" value={form.taxRate} onChange={(e) => setForm({ ...form, taxRate: e.target.value })} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.75rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            <label htmlFor="soldByWeight" style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>Sold by weight (price per unit weight)</label>
          </div>
